...
- The parameters related to RabbitMQ and database connection, that are passed through environment variables, should be added to `appsettings.json` which is the configuration file for the deployed service that have to be placed in its folder. Also, there is `appsettings.Production.json` in the source code folder which we want to merge into that settings.
